...
##Visualizing Location of Reported Temperatures in California
...
```
###This histogram is very cluttered along the X axis with city names. Instead of a histogram I would like to see these cumulative averages per city plotted on a map of California, because I think these higher temperatures might be related to region.
```{r}
...
##Other Plotting of Temperature Data
...
all_data %>%
  ...
  labs(title = "Yearly Average of Days Temperatures Above 105(F) Across Cities", x= "Year", y= "Average Number of Days Above 105(F)")
```
